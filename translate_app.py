...
from config import translation_langs, domain
...
def format_markdown(d, languages_selected):
    str_mkdown = ""

    str_mkdown += "# Input text\n"

    # getting input
    input_text_list = d['input']['text']
    toxic_words_input = d['input']['toxicity']['words']
    ...
    # languages detected
    lang_detected_list = d['input']['lid']['lang']
    lang_str = get_input_lang(input_text_list, lang_detected_list)
    str_mkdown += "{}\n".format(lang_str)

    str_mkdown += "# Translations\n"

    for key in languages_selected:
        lang = key
        trans_list = d['result'][lang]['text']
        toxic_words_input = d['result'][lang]['toxicity']['words']
        
        # formatting input string
        trans_list = mark_toxic_words(trans_list, toxic_words_input)
        result_text_str = list_to_string(trans_list, separator='. ')
        
        str_mkdown += "### {}\n".format(lang)
        str_mkdown += "{}\n\n".format(result_text_str)
    return str_mkdown

endpoint = 'translate'

st.title('Translation with NLLB')

languages_selected = st.multiselect('Select Language to translate',
                                            translation_langs, default=translation_langs[0:2])
languages_selected_str = list_to_string(languages_selected, separator=',')
...
button = button_col.button('Translate')
...
if button:
    ...
    req_body = {'input_text': text_input, 'langs_out': languages_selected_str}
    resp = requests.post(domain+endpoint, json=req_body)
    ...
    # markdown response
    str_mkdown = format_markdown(d, languages_selected)
